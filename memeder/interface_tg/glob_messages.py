
msg_g0 = \
        """
Hi there,
Meme Dating team on the line! We received your feedback and present you v 0.2. What's new:\n
- User's photo 🌠  and bio 🖌\n
- Dating preferences (show me just girls/boys/all/memes) 🤦‍♀️🙎‍♂️\n
- Better memes recommendation system !💥\n
> Soon, we will add .gif and .mp4 support so you will be able to enjoy better memes.
"""


msg_g1 = \
        """
Hello there,

Thanks for being part of the community. Currently, it's 350 users and 12 000 memes reactions,
but it's time to move on 🚀

We collect feedback and present an updated interface for our bot.
What's new:

📊 Percentage of memes compatibility

🎯 Goals for dating

🌠 New format of photo and bio

🔁 Renew all the buttons

We work hard for you and find inspiration in your stories. We know that at least four people go to date via our bot! Congratulations 💥

Our goal is to keep developing fun and cool. For this reason, we announce "dating challenges." The award ceremony will take place in a bar in the center of Moscow.🥂

The details of the first challenge and prizes will be announced tomorrow. Keep in touch.

With love 💓
Meme Dating Team
"""


msg_g2 = \
        """
Привет, многие из вас не могли посмотреть мэтчи из-за бага в системе. 

Мы все починили, дело не в твоем чувстве юмора 😎

 Начинай смотреть новые профили по кнопке "Matching 🎰"

- Еще чуть-чуть улучшили рекоммендательную систему📊

- На днях добавим гифки и короткие видео помимо мемов и улучшим персональное предложение контента💯

- Создали гугл-форму для фидбека и предложений по улучшению 

https://forms.gle/cqmBhKzrbU4uCahs7

- Завели инсту: memes_dating

Завтра сообщим о нашем первом конкурсе в инсте и вечеринке, на которой наградим победителей. Подписывайся;)
"""


msg_g3 = \
        """
Привет,
как и обещали - выкладываем конкурс. Условия таковы:

1) Быть подписанным на инсту: memes_dating;

2) Написать оригинальное сообщение для знакомства;

3) Сделать скриншот и выложить в сторис с упоминанием нашего аккаунта.

Мы организуем первую вечеринку среди наших пользователей. Мы сделали ее для вас💥

26 февраля
с 23.00 до 05.00

вас ждёт:
- просторный двухэтажный лофт
- алкоголь
- еда
- бирпонг
- танцевальная площадка
- новые знакомства💖
- и многое другое

По вопросам покупки билетов пишите @cathrinass
"""
